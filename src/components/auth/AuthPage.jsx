...
export default function AuthPage({ currentRoute, onNavigate }) {
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#06060a] text-light overflow-x-hidden selection:bg-gold/30 selection:text-gold">
      {/* ── FULL SCREEN CONTINUOUS BARTENDER VIDEO BACKGROUND ── */}
      <div className="fixed inset-0 z-0 h-full w-full pointer-events-none">
        <BartenderVideoPanel />
      </div>

      {/* ── FOREGROUND CONTENT LAYER OVERLAYING VIDEO ── */}
      <div className="relative z-10 flex min-h-screen w-full flex-col justify-between p-4 sm:p-6 md:p-8">
        {/* Top Navbar with Return Link */}
        <div className="flex items-center justify-between w-full max-w-7xl mx-auto">
          <button
            type="button"
            onClick={() => onNavigate('/')}
            className="inline-flex items-center gap-2 rounded-full border border-gold/20 bg-black/60 px-4 py-1.5 font-inter text-xs text-smoke/90 hover:text-[#FFD700] hover:border-[#FFD700]/50 hover:bg-black/80 backdrop-blur-md transition-all duration-200"
          >
            <ArrowLeft size={14} />
            <span>Return to the Bar</span>
          </button>

          <div className="hidden sm:flex items-center gap-2 rounded-full border border-gold/15 bg-black/40 px-3.5 py-1 backdrop-blur-md">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse" />
            <span className="font-mono text-[10px] text-smoke">The Speakeasy is Open</span>
          </div>
        </div>

        {/* ── 2-COLUMN MAIN VIEWPORT GRID ── */}
        <div className="grid grid-cols-1 md:grid-cols-2 w-full max-w-7xl mx-auto items-center my-auto py-6 gap-8">
          {/* Left Column: Transparent (Bartender video shines through completely unhindered) */}
          <div className="hidden md:block pointer-events-none" />

          {/* Right Column: Overlaying Smoked Glass Authentication Card */}
          <div className="flex justify-center md:justify-end w-full">
            <motion.div
              ...
              } rounded-2xl border border-[rgba(255,215,0,0.12)] bg-[rgba(10,10,18,0.80)] p-6 sm:p-8 shadow-[0_20px_70px_rgba(0,0,0,0.95),0_0_35px_rgba(255,215,0,0.08)] backdrop-blur-[14px]`}
            >
              ...
              <div className="mb-6 flex items-center justify-between border-b border-[#1a1a2e] pb-4">
        ...
        <div className="w-full text-center font-mono text-[10px] text-smoke/50 py-2">
